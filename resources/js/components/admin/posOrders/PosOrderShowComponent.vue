                ...
                <div class="flex flex-wrap gap-3">
                    ...
                    <div class="relative">
                        <select v-model="order_status" @change.prevent="orderStatus($event)"
                            class="text-sm capitalize appearance-none pl-4 pr-10 h-[38px] rounded border border-primary bg-white text-primary">
                            <option v-for="orderStatus in filteredOrderStatusObject" :value="orderStatus.value">
                                {{ orderStatus.name }}
                            </option>
                        </select>
                        <i
...
export default {
    ...
    computed: {
        ...
        filteredOrderStatusObject: function () {
            ...
            if (this.order.order_type === orderTypeEnum.DELIVERY) {
                const preparedIndex = statuses.findIndex(status => status.value === orderStatusEnum.PREPARED);
                if (preparedIndex !== -1) {
                    statuses.splice(preparedIndex + 1, 0, {
