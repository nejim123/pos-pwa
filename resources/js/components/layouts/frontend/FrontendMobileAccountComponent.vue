<template>
    <div v-if="logged" id="user-profile-dropdown-box"
        ...
        <div class="flex items-center gap-3 p-4 mb-2">
            <figure
                class="flex-shrink-0 relative z-10 w-[68px] h-[68px] rounded-full border-2 border-dashed border-white bg-gradient-to-t from-[#FF7A00] to-[#00B3A5] before:absolute before:inset-0 before:-z-10 before:rounded-full before:scale-[1.03] before:bg-white">
            ...
            <button @click.prevent="closeCanvas('user-profile-dropdown-box')"
                class="fa-solid fa-xmark absolute top-4 right-4 text-white bg-[#FB4E4E] xmark-btn"></button>
        </div>

        <nav class="px-4">
            <router-link v-if="profile.role_id !== enums.roleEnum.CUSTOMER && Object.keys(authDefaultMenu).length > 0"
                :to="{ path: '/admin/' + authDefaultMenu.url }" v-on:click="linkClick"
                class="paper-link transition w-full flex items-center gap-3.5 py-3 border-b last:border-none border-[#EFF0F6]">
                <i class="lab-font-size-17" :class="authDefaultMenu.icon"></i>
                <span class="text-sm leading-6 capitalize">{{ $t('menu.' + authDefaultMenu.language) }}</span>
            </router-link>
            ...
            <button @click="changeLanguageModal()" type="button"
                class="paper-link transition w-full flex items-center gap-3.5 py-3 border-b last:border-none border-[#EFF0F6]">
