<template>
    ...
    <header class="shadow-xs bg-white ff-header" ref="ffHeader">
        <div class="container flex flex-col lg:flex-row items-center justify-between">
            ...
            <div class="flex flex-col items-center justify-end sm:gap-3 w-full sm:mt-4 lg:flex-row lg:w-fit lg:mt-0">
                ...
                <div v-else class="dropdown-group">
                    ...
                    <div
                        ...
                        <div class="flex items-center gap-3 p-4 mb-2">
                            <figure
                                class="flex-shrink-0 relative z-10 w-[68px] h-[68px] rounded-full border-2 border-dashed border-white bg-primary before:absolute before:inset-0 before:-z-10 before:rounded-full before:scale-[1.03] before:bg-white">
                                <a
                        ...
                        <nav class="px-4">
                            <router-link
                                v-if="profile.role_id !== enums.roleEnum.CUSTOMER && Object.keys(authDefaultMenu).length > 0"
                                :to="{ path: '/admin/' + authDefaultMenu.url }"
                                ...
                                <span class="text-sm leading-6 capitalize">{{ $t('menu.' + authDefaultMenu.language)
                                    }}</span>
                            </router-link>
